...
def subject_into_csv_vertex(nr_vertices,hemi,truesubjects,WhichModelType,labels,networkborders,corr_mat_dir,targetdir,nameslhrois,namesrhrois,dataframe2,source_dir=None):
  #creates for each vetex a table containg correlations of vertex time series with time series of selected parcels
  name=['Subject','Zygosity','Mother_ID']
  nonvertexdat=np.zeros((len(truesubjects),3),dtype=object)
